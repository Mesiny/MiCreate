            ...
            <div class="custom-style" style="padding-top: 0px !important;">
                <el-segmented v-model="seg_value2" :options="type2list[type1]" @change="handleCommand2" block>
                    <template #default="scope">
                        <div style="text-align: center;padding: 0px 10px;font-size: 15px;">{{ map[scope.item] }}</div>
                    </template>
                </el-segmented>
            </div>
            <div class="materials">
...
export default {
    ...
    methods: {
        ...
        search() {
            api.getList(1, 12, this.type1, this.type2, this.search_input).then(res => {
        ...
        handleCurrentChange(val) {
            ...
            console.log(`当前页: ${val}`);
            this.curpage = val;
            api.getList(val, 12, this.type1, this.type2, this.search_input).then(res => {
                this.$refs.loadingRef.endLoading();
                let cardList = res.list;
                this.totalpage = res.totalPage;
                this.totalcount = res.totalCount;
