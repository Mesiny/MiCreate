...
const api = {
    ...
    // 4. v2 获取素材列表
    async getList2(page, limit, type = '都市角色', key = '') {
        if (type == undefined || type == '') {
            console.log('类型为空');
            return [];
        }
        const res = await axios({
            url: `http://www.95manhua.com/api/materials/category/${type_dict[type]}?page=${page}&per_page=${limit}`,
            method: 'get'
        });
        ...
        for (let i = 0; i < data.result.items.length; i++) {
            pages.push({
                // atob(res.data.page.list[i].photoPath)
                "id": data.result.items[i].id,
                "fileName": data.result.items[i].title,
                "photoPath": `http://www.95manhua.com/static/${data.result.items[i].previewFilePath}`,
                "type": type,
                "createTime": data.result.items[i].createdAt,
            });
            // console.log(pages[i].photoPath);
        }
        data.list = pages;
        console.log('getList2', res);
        data.pageSize = data.result.per_page;
        data.totalCount = data.result.total;
        data.totalPage = data.result.pages;
        data.currPage = data.result.page;
        return data;
    },
    // 5. v2 获取素材的Psd
    async getPsd2(id, type = '') {
        if (id == undefined || id == '') { console.log('id错误'); return null }
        // console.log(`https://mxjbh.cn/mxjbh/${kind_dict[type1]}info/${id}`);
        const res = await axios({
            url: `http://www.95manhua.com/api/materials/${id}/download`,
            method: 'get'
        });
        console.log('getPsd-res', res.data);
        const data = res.data.result;
        if (data === undefined) return '';
        console.log('getPsd2', data);

        // const psd_url = `http://mxjbh.oss-cn-beijing.aliyuncs.com/${decodeBase64(address)}`;
        return data.downloadUrl;
    },
    // 6. search
    async search(page, limit, key = '') {
        const res = await axios({
            url: `http://www.95manhua.com/api/materials/search?keyword=${key}&page=${page}&per_page=${limit}`,
            method: 'get'
        });
        ...
        for (let i = 0; i < data.result.items.length; i++) {
            pages.push({
                ...
                "fileName": data.result.items[i].title,
                "photoPath": `http://www.95manhua.com/static/${data.result.items[i].previewFilePath}`,
                "type": data.result.items[i].categoryName,
                "createTime": data.result.items[i].createdAt,
            });
            // console.log(pages[i].photoPath);
        }
